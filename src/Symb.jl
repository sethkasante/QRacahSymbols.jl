...
@inline function _ensure_capacity!(exps::Vector{Int}, n::Int)
    ...
    if n > old_len
        resize!(exps, n)
        # resize! leaves uninitialized memory; we must explicitly zero it
...
"""
    mul_qfact(M::CycloMonomial, n::Int, power::Int=1)

Mutates the internal array of M and returns an updated CycloMonomial struct.
Completely avoids building intermediate arrays.
"""
function mul_qfact(M::CycloMonomial, n::Int, power::Int=1)
    ...
    # Return new stack-allocated struct pointing to the SAME mutated array
    return CycloMonomial(M.sign, new_z_pow, M.exps)
end

"""
    mul_qint(M::CycloMonomial, n::Int, power::Int=1)

Mutates the internal array by [n]_q^{power}.
"""
...
function q6jsummand_symb_first(z, α1, α2, α3, α4, β1, β2, β3)
...
function q6jseries_symb(j1, j2, j3, j4, j5, j6)::Vector{CycloMonomial}
    ...
    if z_min <= z_max
        # 1. Base Case: Build the first summand
        term = q6jsummand_symb_first(z_min, α1, α2, α3, α4, β1, β2, β3)
        push!(S_z, term)

        # 2. Iterative Step: Compute subsequent terms via the Hypergeometric Ratio
...
function q3jsummand_symb_first(z, α1, α2, β1, β2, β3)
