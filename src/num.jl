...
function logqn_table(k::Int, T::Type, prec::Int)::Vector{T}
    N = k + 2
    # big(π) for when T = BigFloat
    θ = (T === Float64) ? (pi / N) : (big(π) / BigFloat(N))
    logsinθ = log(sin(θ))
...
qracah6j_numeric(j1::Spin, j2::Spin, j3::Spin, j4::Spin, j5::Spin, j6::Spin, k::Int; T::Type{<:AbstractFloat}=Float64, prec::Int=256) = 
...
qracah3j_numeric(j1::Spin, j2::Spin, j3::Spin, m1::Spin, m2::Spin, k::Int; T::Type{<:AbstractFloat}=Float64, prec::Int=256) = 
